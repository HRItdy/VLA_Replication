...
def convert_rotation_vector_to_ortho6d(rotvec):
    """
    Convert rotation vectors to ortho6d representations.
    
    Args:
        rotvec: numpy array of shape (batch_size, 3) representing rotation vectors
               (axis-angle representation)
        
    Returns:
        ortho6d: numpy array of shape (batch_size, 6) containing ortho6d representation
    """
    # First, convert rotation vector to rotation matrix using scipy
    # Handle batch dimension
    if len(rotvec.shape) == 1:
        rotvec = rotvec[None, :]  # Add batch dimension if missing
    
    batch_size = rotvec.shape[0]
    rotation_matrices = np.zeros((batch_size, 3, 3))
    
    for i in range(batch_size):
        # Use scipy's Rotation class to convert rotation vector to matrix
        rotation_matrices[i] = R.from_rotvec(rotvec[i]).as_matrix()
    
    # Then convert rotation matrix to ortho6d
    ortho6d = compute_ortho6d_from_rotation_matrix(rotation_matrices)
    
    return ortho6d


def convert_ortho6d_to_rotation_vector(ortho6d):
    """
    Convert ortho6d representations back to rotation vectors.
    
    Args:
        ortho6d: numpy array of shape (batch_size, 6)
        
    Returns:
        rotvec: numpy array of shape (batch_size, 3) containing rotation vectors
    """
    # First convert ortho6d to rotation matrix
    rotation_matrices = compute_rotation_matrix_from_ortho6d(ortho6d)
    
    # Convert rotation matrix to rotation vector
    batch_size = ortho6d.shape[0]
    rotvec = np.zeros((batch_size, 3))
    
    for i in range(batch_size):
        # Use scipy's Rotation class to convert matrix to rotation vector
        rotvec[i] = R.from_matrix(rotation_matrices[i]).as_rotvec()
...
if __name__ == "__main__":
    ...
    # Create sample rotation vectors
    batch_size = 2
    ...
    print("Original rotation vectors:")
    print(rotation_vectors)
    
    # Convert to ortho6d
    ortho6d = convert_rotation_vector_to_ortho6d(rotation_vectors)
    print("\nOrtho6d representations:")
    print(ortho6d)
    
    # Convert back to rotation vectors to verify
    recovered_rotvec = convert_ortho6d_to_rotation_vector(ortho6d)
    print("\nRecovered rotation vectors:")
    print(recovered_rotvec)
    
    # Calculate error
    error = np.linalg.norm(rotation_vectors - recovered_rotvec, axis=1)
    print("\nError in conversion (should be close to zero):")
    print(error)