...
from diffusion_policy.real_world.dataset_conversion import convert_replay_to_h5py
...
class RealScrewDriverDataset(BaseImageDataset):
    def __init__(self,
            shape_meta: dict,
            dataset_path: str,
            horizon=1,
...
def _get_replay_buffer(dataset_path, shape_meta, store):
    ...
    with threadpool_limits(1):
        ...
        convert_replay_to_h5py(
            replay_buffer=replay_buffer,
            output_dir="/home/daiying/diffusion_policy_with_kovis/diffusion_policy/data/hdf5",
            task_name="screwdriver",
            dataset_path=dataset_path,
            camera_names=['camera_0'],
        )
        # replay_buffer, h5_file = real_data_to_replay_buffer(
        #     dataset_path=dataset_path,
        #     out_zarr_store=None,  # Uses memory store
        #     out_hdf5_path='output.h5',  # Saves to disk
        #     out_resolutions=out_resolutions,
        #     lowdim_keys=lowdim_keys + ['action'],
        #     image_keys=rgb_keys
        ...
        # Don't forget to close the HDF5 file when done
        h5_file.close()

        # # store the dataset as h5py dataset file
        # convert_replay_to_h5py(replay_buffer=replay_buffer,
        #                        data_dir=dataset_path+"h5py",
        #                        task_name="real_screwdriver",
        #                        camera_names=["camera_0"]
        #                        )
