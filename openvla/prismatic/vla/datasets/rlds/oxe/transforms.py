...
def robo_ur_screwdriver_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    # gripper action is in -1 (open)...1 (close) --> clip to 0...1, flip --> +1 = open, 0 = close
    num_actions = tf.shape(trajectory["action"])[0]
    gripper_action = tf.fill(
        (num_actions, 1), 
        tf.cast(1, trajectory["action"].dtype)  # Match dtype with trajectory["action"]
    )
    trajectory["action"] = tf.concat(
        [
            trajectory["action"][:, :6],
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "robo_ur_screwdriver": robo_ur_screwdriver_transform,
}
